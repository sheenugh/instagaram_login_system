...
class InstagramLoginSystemComponents:
    def __init__(self, root):
        ...
        # Load the success sound (ensure this is a WAV file)
        try:
            self.success_sound_for_login = pygame.mixer.Sound("bumalik_kana.mp3")
            self.failure_sound = pygame.mixer.Sound("bumalik_kana.mp3")  # Add your failure sound here
        ...
        self.incorrect_username_lyrics = [
            "..........\n"
            "Walang naganap na pagbabago\n"
            "Subukan mong muli\n"
            "..........\n"
            "Huwag mawalan ng pag-asa\n"
            "Bumalik ka at subukan ulit"
        ]

        self.incorrect_password_lyrics = [
            "..........\n"
            "Tila may mali sa iyong password\n"
            "Pakisuri at subukan muli\n"
            "..........\n"
            "Tiyakin ang mga detalye\n"
            "Huwag mawalan ng pag-asa"
        ]
    ...
    def login(self):
        ...
        if username in valid_credentials:
            if password == valid_credentials[username]:
                ...
                print("Login successful. Playing sound.")
                self.success_sound_for_login.play()
                self.show_success_lyrics_window()  # Show success lyrics if specific username
                self.username.set("")
                self.password.set("")
            else:
                # Invalid password
                self.handle_login_failure("Invalid password. Please try again.", self.incorrect_password_lyrics)
        else:
            # Invalid username
            self.handle_login_failure("Incorrect username. Please try again later.", self.incorrect_username_lyrics)

    # - Handle login failure
    def handle_login_failure(self, error_message, lyrics):
        messagebox.showerror("Error", error_message)
        self.failure_sound.play()  # Ensure you have a failure sound file
